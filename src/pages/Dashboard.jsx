...
export default function Dashboard() {
  ...
  async function loadData() {
    // Fetch all data from Supabase using user_id only — nothing from localStorage
    const { data: userData } = await supabase
  ...
  return (
    <div className="dashboard">
      ...
      <style jsx="true">{`
        ...
        .logout-btn:hover { transform: scale(0.96); background: #1f1f1f; box-shadow: 0 2px 6px rgba(0,0,0,0.05); }
        ...
        .discount-card:hover { transform: translateY(-2px); box-shadow: 0 14px 28px rgba(0, 0, 0, 0.04); background: #fefefe; }
        .discount-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; }
        .discount-badge {
          font-size: 11px;
          text-transform: uppercase;
        ...
        .discount-icon-wrapper svg { width: 28px; height: 28px; }
        .code-container {
          display: flex;
          align-items: center;
          justify-content: space-between;
