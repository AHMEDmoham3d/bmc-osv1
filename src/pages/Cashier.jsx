...
export default function Cashier() {
  ...
  async function handlePlaceLogin() {
    if (!placeCode) { setError("ادخل كود المكان"); return; }
    setLoading(true); setError("");
    const { data } = await supabase.from("places").select("*").eq("cashier_code", placeCode).single();
    if (!data) { setError("كود المكان غلط"); setLoading(false); return; }
    setPlace(data); setLoading(false);
  }

  async function handleFindUser() {
    if (!phone) { setError("ادخل رقم موبايل العميل"); return; }
    setLoading(true); setError("");
    const { data: userData } = await supabase.from("users").select("*").eq("phone", phone).single();
    if (!userData) { setError("المستخدم ده مش مسجل"); setLoading(false); return; }
    setUser(userData);
    ...
    if (existingCode) {
      ...
      const { data: newCode } = await supabase.from("discount_codes").insert({
        user_id: userData.id,
        code,
        place: place.name,
        place_id: place.id,
  ...
  async function handleConfirm() {
    if (place.type === "restaurant" && !item) { setError("ادخل اسم الأكلة"); return; }
    if (!price) { setError("ادخل السعر"); return; }
    if (place.type === "court" && !hours) { setError("ادخل عدد الساعات"); return; }
    ...
    await supabase.from("orders").insert({
      ...
      place_id: place.id,
      place: place.name,
      item: place.type === "restaurant" ? item : place.type === "court" ? `${hours} ساعة` : place.type === "gym" ? item : item,
      price: Number(price),
    ...
    return (
      <div className="cashier-page">
        <div className="cashier-card">
          ...
          <p className="subtitle">دخول المكان</p>
          <div className="form-group">
            <label className="form-label">
              <StoreIcon />
              <span>كود المكان</span>
            </label>
            <input
      ...
      <div className="cashier-page">
        <div className="cashier-card success-card">
          <div className="success-box">
            ...
            <p className="success-title">تم تسجيل العملية</p>
            <p className="success-sub">وُفِّر {place.discount_amount} جنيه لـ {user?.name}</p>
          </div>
        </div>
        <style jsx="true">{css}</style>
      </div>
    ...
    <div className="cashier-page">
      <div className="cashier-card">
        <div className="header-row">
          <h1 className="logo-small">يومي</h1>
          <div className="place-badge">{place.name}</div>
        </div>
        <p className="discount-info">
          <TagIcon />
          <span>الخصم: {place.discount_amount} جنيه • {getTypeLabel(place.type)}</span>
        </p>
        ...
        {step === "phone" && (
          <>
            ...
            <button className="submit-btn" onClick={handleFindUser} disabled={loading}>
              {loading ? <span className="loading-spinner"></span> : "التالي"}
            </button>
          </>
        )}
          ...
          <>
            <div className="user-card">
              ...
              <div className="discount-code-chip">
                <span className="chip-label">كود الخصم</span>
            ...
            {place.type === "restaurant" && (
              <div className="form-group">
                <label className="form-label">الأكلة أو الطلب</label>
                <input
                  className="form-input"
                  placeholder="مثال: برجر + بطاطس"
                  value={item}
            ...
            {place.type === "gym" && (
              <div className="form-group">
                ...
                <select className="form-input" value={item} onChange={e => setItem(e.target.value)}>
                  <option value="">اختار نوع الاشتراك</option>
                  <option value="حصة واحدة">حصة واحدة</option>
            ...
            {place.type === "beach" && (
              <div className="form-group">
                <label className="form-label">نوع الباكدج</label>
                <select className="form-input" value={item} onChange={e => setItem(e.target.value)}>
                  <option value="">اختار الباكدج</option>
                  <option value="دخول فقط">دخول فقط</option>
                  <option value="دخول + كرسي">دخول + كرسي</option>
                  <option value="دخول + كرسي + مشروب">دخول + كرسي + مشروب</option>
                </select>
              </div>
            )}
            ...
            <button className="submit-btn" onClick={handleConfirm} disabled={loading}>
              {loading ? <span className="loading-spinner"></span> : "تأكيد وتطبيق الخصم"}
            </button>
            <button
              className="back-btn"
              onClick={() => { setStep("phone"); setPhone(""); setUser(null); setError(""); }}
            >
              رجوع
            </button>
          </>
